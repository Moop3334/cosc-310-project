...
def auth_user(username:str, password:str):
    user = get_user_by_username(username) #get user from storage, should return an error if user isn't found
    if not user:
        verify_password(password, DUMMY_HASH)
        return False
    if not verify_password(password, user.password_hash):
        return False
    return UserLogin(username=username, password_hash=password_hash)
